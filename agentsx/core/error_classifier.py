"""Centralized error classification for provider errors.

Maps ``ProviderError`` instances to a structured ``ClassifiedError`` with
a ``FailoverReason`` enum and ``RecoveryAction`` dataclass, enabling
intelligent recovery decisions (auto-compaction, retry, fallback)
throughout the agent loop.

Priority order (highest first):
1. Thinking signature mismatch
2. Context overflow
3. HTTP status codes (401, 402, 429, 403, 5xx)
